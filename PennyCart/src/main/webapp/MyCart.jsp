...
<c:forEach var="buyprod" items="${sessionScope.cartList}" varStatus="status">
	<tr>
		...
		<td><input type="number" id="pQuant${status.index}" type="text" maxlength="2" max="${buyprod.quantity}" value="1" onchange="total('total${status.index}','pPrice${status.index}','pQuant${status.index}')" onkeyup="total('total${status.index}','pPrice${status.index}','pQuant${status.index}')"></td>
		<td>$<input type="text" id="total${status.index}" value="${buyprod.price}" readonly="readonly"></td>
