...
<body>
	...
	<table align="center" style="color: #DA621E;">
		...
		<c:forEach var="buyprod" items="${sessionScope.cartList}"
			...
			<tr>
				...
				<td><input type="number" name="inCount"
					id="pQuant${status.index}" type="text" maxlength="2" min="1"
					max="${buyprod.quantity}" value="${buyprod.count}"
					onchange="calcGrandTotal('total${status.index}','pPrice${status.index}','pQuant${status.index}','prevCount${status.index}','${buyprod.quantity}')"
					onkeyup="calcGrandTotal('total${status.index}','pPrice${status.index}','pQuant${status.index}','prevCount${status.index}','${buyprod.quantity}')"></td>
	...
	<form action="UpdateCart" method="post" name="updForm">
		...
		<table>
			...
			<c:forEach var="updProd" items="${sessionScope.cartList}"
				varStatus="status">
				<tr>
					<td><input type="hidden" id="1pName${status.index}"
						name="pCartName" value="${updProd.productName}"></td>
					<td><input type="hidden" id="1pPrice${status.index}"
						type="text" value="${updProd.price}" readonly="readonly"></td>
					<td><input type="hidden" name="pCartCount"
						id="1pQuant${status.index}" type="text" maxlength="2"
						max="${buyprod.quantity}" value="1"
						onchange="total('total${status.index}','pPrice${status.index}','pQuant${status.index}')"
						onkeyup="total('total${status.index}','pPrice${status.index}','pQuant${status.index}')"></td>
					<td><input type="hidden" id="1total${status.index}"
						value="${updProd.price*updProd.count}" readonly="readonly"></td>
				</tr>
			</c:forEach>
		</table>
