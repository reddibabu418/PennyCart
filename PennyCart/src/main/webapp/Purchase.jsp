...
<body>
	...
	<table align="center" style="color: #DA621E;">
		...
		<c:forEach var="buyprod" items="${paramValues.checkButton}"
			varStatus="status">
			<tr>
				<td id="pName${status.index}">${sessionScope.productsList[buyprod-1].productName}</td>
				<td><input id="pPrice${status.index}" type="text"
					value="${sessionScope.productsList[buyprod-1].price}"
					readonly="readonly"></td>
				<td><input type="number" id="pQuant${status.index}" type="text"
					maxlength="2"
					max="${sessionScope.productsList[buyprod-1].quantity}" value="1"
